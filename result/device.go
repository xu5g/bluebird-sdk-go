...
type DeviceEntity struct {
	...
	ExerciseTarget    []ExerciseTarget `json:"exercise_target"`    //运动目标配置
}
...
type HealthRemind struct {
	...
	Target       int64  `json:"target"`         //目标值，根据不同功能选用
}

// 运动目标
type ExerciseTarget struct {
	TargetTime     int64   `json:"target_time"`     //目标时长，单位分钟，（5-240分钟）int类型
	TargetDistance float64 `json:"target_distance"` //运动距离目标，单位公里，（0-100公里）int类型
	TargetCalorie  int64   `json:"target_calorie"`  //消耗热量，单位千卡，（100-5000千卡）int类型
